...
- [ ] P3: facts/provenance/chunks из одной IR; staging/activation, crash/retry/reprocess, UI качества/источника. Staging/activation и retry сделаны в [срезе P3](#срез-p3-processing-revision-и-активация-26-сентября-2026); режим reprocess, подтверждение настроек индекса и сохранение снимка при правке сделаны там же; остаются UI качества/источника и crash injection.
...
- [x] P4: согласованный backup оригиналов/БД/IR/review, восстановление на чистом Compose и проверка полноты: `scripts/archive_backup.py`, опциональное шифрование GnuPG, реальная Docker-проверка на synthetic seed в CI. Полный стек с AI/Ollama после восстановления и личная установка владельцем ещё не проверены.
...
- [ ] P0: независимый held-out corpus для baseline (P3 `processing_revisions`, активация, staging индекса, режим reprocess и подтверждение настроек индекса сделаны в срезе P3 ниже).

## Срез P3: processing revision и активация, 26 сентября 2026

- [x] `processing_revisions` с неизменяемой идентичностью, одной `ACTIVE` на документ и явной миграцией; факты новой обработки сохраняются неактивными в своей ревизии.
- [x] Индекс ставится по ревизии (`revisionId` в `/internal/index`, манифест), прежние chunks не трогаются до активации; `/internal/prune` после неё.
- [x] Активация одной транзакцией: факты, метаданные документа, timeline, прежняя ревизия `SUPERSEDED`; ответы по архиву читают только chunks активной ревизии каждого документа.
- [x] Сбой индексации: повтор без повторного извлечения; неверный манифест или окончательный сбой дают `FAILED` без смешения версий; правка во время подготовки ставит новую обработку.
- [x] UI документа сообщает, какая версия обработки показана, пока готовится новая или последняя не удалась.
- [x] Выбор режима reprocess: «текущий текст» из сохранённой стадии разбора или «заново разобрать оригинал»; недоступный режим отклоняется, UI предлагает только доступные.
- [x] Индексатор подтверждает настройки индекса из recipe (включая digest embedding-модели) и отказывает до записи; backend сверяет манифест, расхождение даёт `FAILED` без смешения версий.
- [x] Правка текста не выключает факты сразу: факты и хронология прежней версии видны с пометкой до активации новой; ответы по архиву документ не используют.
- [x] Исправлено: обработка очищала `storagePath`, и оригинал переставал открываться.
- [ ] UI качества/источника; crash injection на уровне процесса и параллельный reprocess под нагрузкой; восстановление уже очищенных путей к оригиналам.
...
- [x] Зафиксировать решение автора и сверить его с кодом: [целевая архитектура, статус и разрывы](docs/TARGET_ARCHITECTURE.md). Это проектирование, не реализация.
- [x] Runtime модели: универсальный OpenAI-совместимый коннектор (решение автора 25.09).
- [ ] Ответ автора на остальные вопросы: машина для endpoint Qwen 3.8 27B, мультимодальная сборка или отдельная vision-модель, роль веб-кабинета, папка входящих.
- [ ] A0: закрыть предусловия безопасности из ревью 25.09 и P0–P2 pipeline.
- [ ] A1: штатный OpenAI-совместимый коннектор; измерить Qwen 3.8 27B с контекстом 180k на целевой машине: latency, память, стабильность.
- [ ] A2: изолированный контейнер opencode без egress; Private Archive MCP только на чтение; тесты границы.
- [ ] A3: слой выводов агента, `add_material`/`record_finding`/`propose_fact_review`, review в UI.
- [ ] A4: справочная библиотека и skill с manifest лицензий.
- [ ] A5: исследовательский цикл с облачным запросом, сверкой ответа и цепочкой итераций.
- [ ] A6: vision/OCR в ingestion по политике согласования.
- [ ] После подтверждения автором уточнить формулировку AGENTS.md о MCP: публичный MCP остаётся синтетическим, Private Archive MCP — отдельный локальный сервер.
