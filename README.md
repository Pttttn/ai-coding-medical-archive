...
4. Создайте заметку или загрузите PDF с текстовым слоем. Дождитесь локальной обработки; сканы получают понятный статус OCR_REQUIRED, OCR в MVP нет.
...
Backend integration tests используют отдельную PostgreSQL БД из `TEST_DATABASE_URL`, никогда личную базу. Обычные CI-тесты не требуют GPU или LLM API, используют явно тестовые адаптеры. Реальная оценка Ollama выполняется отдельно; её команды, измеренные результаты, параметры chunking и ограничения находятся в `docs/VALIDATION.md` и `docs/evaluation/`. GitHub Actions проверяет типы/lint, тесты и сборку всех контейнеров.
