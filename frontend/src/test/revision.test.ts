...
describe('processing revision notice', () => {
  ...
  it('does not present the old snapshot as current after a text edit', () =>
    expect(revisionNotice({ status: 'INDEXING', textVersion: 2, textRevisions: [{ id: 't2', version: 2, createdAt: '' }, { id: 't1', version: 1, createdAt: '' }], processingRevision: { active: view('a'), prepared: view('b') } })).toBeNull());
