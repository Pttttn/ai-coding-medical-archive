...
import { assertions, date, dateInput, dateTime, documentTypes, factTypes, hasFactCorrection, isProcessing, reviews, tagsFromInput, validateDocument, validateTags } from '../utils';
...
export function DocumentPage() {
  ...
  return <><Link className="back-link" to="/archive"><ArrowLeft size={16} /> К архиву документов</Link>
    ...
    {doc.processingWarnings && doc.processingWarnings.length > 0 && <Notice variant="warning"><strong>Обратите внимание на результат обработки</strong><ul>{doc.processingWarnings.map((warning, index) => <li key={index}>{warning}</li>)}</ul></Notice>}{isProcessing(doc.status) && <Notice>Обработка выполняется локально. Статус и результаты обновятся автоматически.</Notice>}
    ...
    <section className="summary-panel"><span className="summary-icon"><Sparkles size={22} /></span><div><h2>Краткое содержание <span className="subtle-pill">{doc.isSeed ? 'Синтетическое демо' : 'AI'}</span></h2><p>{doc.summary || 'Краткое содержание появится после обработки документа.'}</p></div></section>
    <div className="archive-tabs document-tabs"><button className={tab === 'facts' ? 'active' : ''} onClick={() => setTab('facts')}>Медицинские факты{doc.facts && <span className="number-badge">{doc.facts.length}</span>}</button><button className={tab === 'text' ? 'active' : ''} onClick={() => setTab('text')}>Текст документа</button><button className={tab === 'history' ? 'active' : ''} onClick={() => setTab('history')}><History size={15} /> История</button></div>
