...
import type { Document, Fact, FactRevision, FactSource, HistoryEvent, Page, ReprocessMode, TextRevision } from '../types';
import { assertions, date, dateInput, dateTime, documentTypes, factTypes, hasFactCorrection, isProcessing, reprocessModeLabels, reviews, revisionNotice, tagsFromInput, validateDocument, validateTags } from '../utils';
...
export function DocumentPage() {
  ...
  const [mode, setMode] = useState<'' | ReprocessMode>('');
  async function action(type: 'delete' | 'restore' | 'reprocess') {
    setBusy(true); setError(''); setSuccess('');
    try { await api(`/documents/${id}${type === 'delete' ? '' : `/${type}`}`, { method: type === 'delete' ? 'DELETE' : 'POST', ...(type === 'reprocess' && mode ? { body: json({ mode }) } : {}) }); setMode(''); setSuccess(type === 'delete' ? 'Документ перемещён в корзину. Его можно восстановить.' : type === 'restore' ? 'Документ восстановлен.' : 'Документ добавлен в очередь обработки.'); state.reload(); }
  ...
  return <><Link className="back-link" to="/archive"><ArrowLeft size={16} /> К архиву документов</Link>
    ...
    {mode === 'ORIGINAL' && <Notice>Текст будет заново получен из оригинала. Если вы правили текст, правка станет предыдущей версией и останется в истории; проверенные факты сохранятся.</Notice>}
