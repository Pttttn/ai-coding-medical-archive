...
| Структурирование при загрузке | **частично** | Source IR `SOURCE_ONLY` (страницы, spans, карта нормализации), сохраняется до вызова модели и переиспользуется при повторе; полный recipe обработки с hash на каждом запуске извлечения; opt-in `lab-rows-v1` (development 80/80, held-out 40/40 × 3); opt-in `clinical-v1` и `clinical-reviewed-v1` для VISIT (gates не пройдены); таблицы PDF с текстовым слоем; processing revision: facts и chunks новой обработки готовятся неактивными, индекс ставится по ревизии, активация одной транзакцией, до неё поиск и ответы читают прежний полный снимок | P3: выбор режима reprocess в UI, подтверждение настроек индекса индексатором, сохранение прежнего снимка при правке текста пользователем; P4; held-out corpus P0; другие типы документов ([план](MEDICAL_DOCUMENT_PIPELINE_PLAN.md)) |
