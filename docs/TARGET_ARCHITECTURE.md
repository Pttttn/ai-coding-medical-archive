...
| Локальная LLM | **частично** | Ollama, дефолт `qwen3.5:2b`; 9B в клинических сериях; `qwen3.8-27b-q8-100k-cuda` проверялась только на сервере mpc1 через опциональный `compose.llama-cpp.yaml` и контексте 100k | 27B с контекстом 180k локально не измерена; CPU-offload 27B остановлен из-за задержек до 425 с. Нужны выбор runtime и оборудования, бюджеты latency/памяти |
...
| Резервная копия и восстановление | **нет** | Описано как обязательное для первого релиза ([PRODUCT_DIRECTION](PRODUCT_DIRECTION.md)) | Backup manifest и проверенное восстановление; с агентом добавляются выводы и история исследований |
...
- DNS-rebinding и CSRF на loopback-портах: allowlist Host и проверка Origin;
- запрет демо-паролей и демо-токенов в личном режиме;
...
| A1 | Выбор runtime и оборудования для Qwen 3.8 27B, контекст 180k; vision-модель | Измерены latency p50/p95, пиковая память и стабильность на выбранной машине; записано в VALIDATION |
...
- Какой runtime для Qwen 3.8 27B: Ollama, llama.cpp или другой; на какой машине он работает постоянно?
