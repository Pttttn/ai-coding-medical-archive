...
| --- | --- | --- | --- |
| Хранение оригиналов, версии текста, факты, provenance, review, аудит | **есть** | NestJS + TypeORM + PostgreSQL, неизменяемые TextRevision, Provenance, FactRevision, AuditEvent ([ARCHITECTURE](../ARCHITECTURE.md)) | Находки ревью о целостности данных закрыты: правка значения всегда CORRECTED, правка во время обработки не пропускает извлечение ([VALIDATION](VALIDATION.md)). Privacy-очистка больше не портит медицинские слова: замены только целыми словами, стоп-лист медицинских терминов, ФИО с отчеством и инициалами скрываются без модели. Настоящее удаление (purge) ещё открыто |
...
| Структурирование при загрузке | **частично** | Source IR `SOURCE_ONLY` (страницы, spans, карта нормализации), сохраняется до вызова модели и переиспользуется при повторе; полный recipe обработки с hash на каждом запуске извлечения; opt-in `lab-rows-v1` (development 80/80, held-out 40/40 × 3); opt-in `clinical-v1` и `clinical-reviewed-v1` для VISIT (gates не пройдены); таблицы PDF с текстовым слоем; processing revision: facts и chunks новой обработки готовятся неактивными, индекс ставится по ревизии, активация одной транзакцией, до неё поиск и ответы читают прежний полный снимок; индексатор подтверждает настройки индекса из recipe и отказывает при расхождении; reprocess с выбором «текущий текст» (без повторного разбора) или «заново разобрать оригинал»; после правки текста факты прежней версии видны с пометкой до обработки новой | P3: UI качества/источника, crash injection и параллельный reprocess под нагрузкой; P4; held-out corpus P0; другие типы документов ([план](MEDICAL_DOCUMENT_PIPELINE_PLAN.md)) |
...
- проверка многострочных цитат, чтобы нельзя было склеить несмежные фрагменты (сделано);
- правка значения факта всегда даёт CORRECTED (сделано);
- правка метаданных или факта во время обработки не пропускает извлечение фактов (сделано);
