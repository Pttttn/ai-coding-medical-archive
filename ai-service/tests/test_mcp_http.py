...
async def test_four_tools_over_real_streamable_http(services, settings, provider):
    (settings.sample_docs_dir / "visit.md").write_text("SYN-CASE-7F29", encoding="utf-8")
    ...
    try:
        async with Client(f"http://127.0.0.1:{port}/mcp") as client:
            ...
            found = await client.call_tool("find_relevant_docs", {"query": "SYN-CASE-7F29", "top_k": 2})
            assert found.data["chunks"][0]["source"] == "visit.md"
            assert all(c[0] == "embed" for c in provider.calls)
            answer = await client.call_tool("ask_question", {"question": "What code?"})
            assert "SYN-CASE-7F29" in answer.data["answer"]
            assert "documentId" not in answer.data["sources"][0]
            assert answer.data["sources"][0]["source"] == "visit.md"
