...
`sanitize_fields` combines deterministic identifier rules, local-model span detection and final deterministic validation. Provider failure, invalid output schema and changes to protected clinical numbers, units, doses, dates or negations block both channels. Public MCP uses `strict=True`: an invalid proposed span (including one absent from the text) or an ambiguous span containing numbers/negation causes a safe refusal. Manual consultation uses `strict=False`: an absent span is ignored, while a numeric/negative possible identifier is left unchanged with a warning requiring manual removal and review. The draft still requires review of the exact content hash before export; this is not a raw fallback on provider failure. Rules redact labelled names, unlabelled Russian full names with a patronymic, surnames with initials and names after an honorific (Dr., Prof., Mr., д-р, проф.), plus absolute, home and relative document paths. Multi-word names are replaced regardless of case; a single name part only as a capitalised, case-sensitive whole word outside a stop-list of medical words and eponyms, so a doctor "Mark Stone" never turns "kidney stone" into a placeholder (the lone surname "Stone" then stays in the text). Model spans are replaced only as whole words; a span shorter than 3 characters, a stop-listed medical word, a span found only inside longer words or one crossing two fields is refused in strict mode and left unchanged with a warning in manual consultation. Plain unlabelled English "First Last" names still depend on the local model. Quasi-identifiers can remain, and passing the synthetic suite does not guarantee full anonymity.
